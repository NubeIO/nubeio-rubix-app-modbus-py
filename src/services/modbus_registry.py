...
class ModbusRegistry(metaclass=Singleton):
    ...
    def add_edit_and_get_connection(self, network: NetworkModel) -> ModbusRegistryConnection:
        registry_key: ModbusRegistryKey = self.get_registry_key(network)
    ...
    def register(self):
        ...
        for network in NetworkModel.query.filter_by(type=self.get_type()):
            self.initialize_network_connections(network)
        logger.info(f"Finished {self.get_type().name} poll registration")

    def initialize_network_connections(self, network: NetworkModel):
